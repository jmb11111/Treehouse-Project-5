  ...
  //builds each card for individuals
   buildCard(){
    ...
    buildModal(){
      ...
      <p>${this.cell}</p>
      <p>Birthday: ${this.dob.toString().slice(0,9)}</p>
      </div>
  ...
  getUsersData()
      .then(results => {
          ...
          $("li").click(function(event){
           ...
           displayModalOnClick($(this).find("p").text());
...
const displayModalOnClick = (selectedEmail) => {
          ...
          currentEmployee = employees.filter(emp => emp.email === selectedEmail)
           currentEmployee.forEach(element => {
            currentEmployeeZ = new Employee(element.name, element.location, element.email, element.picture, element.login, element.cell, element.dob, element.selected);
...
const displayModalOnSearch = (searchInput) => {
          ...
          currentEmployee = employees.filter(emp => emp.name.first.toString().includes(searchInput) ||emp.name.last.toString().includes(searchInput) || emp.login.username.toString().includes(searchInput) );
...
const displayNextModal = () => {
  ...
  currentEmployee.forEach(element => {
    nextEmployeeZ = new Employee(element.name, element.location, element.email, element.picture, element.login, element.cell, element.dob, element.selected);
